...
# Symlink HYPRDE configs into ~/.config and apply optional extras.
# Backups go under $XDG_STATE_HOME/HYPRDE/backups/<run-id>/…

task::run() {
    local policy="${HYPRDE_LINK_POLICY:-backup}" # backup|replace|skip
    ...
    local BACKUP_ROOT="${HYPRDE_BACKUP_DIR:-$STATE_BASE/HYPRDE/backups}"
    ...
    local KEEP="${HYPRDE_BACKUP_KEEP:-10}"
